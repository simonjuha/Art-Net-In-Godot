...
func set_fixture_channels(offset: int, channel_count: int) -> void:
	dmx_channel_offset = offset
...
# se if local data is different from the new data
	var temp_local_dmx_data = dmx_data_array.slice(dmx_channel_offset, total_channels)
